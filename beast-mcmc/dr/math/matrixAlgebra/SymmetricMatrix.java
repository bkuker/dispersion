...
public class SymmetricMatrix extends Matrix {
		...
		for (int i = 0; i < comp.length; i++) {
			for (int j = 0; j < i; j++) {
				if (comp[i][j] != comp[j][i])
					throw new NonSymmetricComponents("Non symmetric components: a[" + i + "][" + j + "]= " + comp[i][j]
							+ ", a[" + j + "][" + i + "]= " + comp[j][i]);
			}
		}
	...
	public Matrix inverse() throws ArithmeticException {
		return rows() < lupCRLCriticalDimension ? new SymmetricMatrix(
				(new LUPDecomposition(this)).inverseMatrixComponents()) : crlInverse();
	}
	...
	private SymmetricMatrix inverse2By2() {
		...
		double inverseDeterminant = 1 / (components[0][0] * components[1][1] - components[0][1] * components[1][0]);
	...
	private static SymmetricMatrix join(Matrix[] a) {
		...
		for (int i = 0; i < p; i++) {
			...
			for (int j = p; j < n; j++)
				newComponents[i][j] = newComponents[j][i] = -a[2].components[j - p][i];
		}
		for (int i = p; i < n; i++) {
			for (int j = p; j < n; j++)
